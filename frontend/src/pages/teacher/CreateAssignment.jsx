...
export const CreateAssignment = () => {
 ...
 <div>
 <h1 className="font-mono text-4xl font-bold text-white mb-2">Assignment Management</h1>
 <p className="text-gray-400 font-mono text-sm tracking-tight">Create and manage academic tasks for your students.</p>
 </div>
 <button
 onClick={() => setModalOpen(true)}
 className="flex items-center gap-2 bg-[#a855f7] text-white px-6 py-3 rounded-sm font-mono text-sm tracking-widest uppercase hover:bg-purple-700 transition-all group"
 >
 ...
 [1, 2, 3].map(i => (
 <div key={i} className="h-64 bg-[#111] border-[#333] focus:border-[#a855f7] animate-pulse border border-[#333]"></div>
 ))
 ) : assignments.length === 0 ? (
 <div className="col-span-full py-20 text-center border-2 border-dashed border-[#333] rounded-sm">
 <BookOpen size={48} className="mx-auto text-[#D2B48C] mb-4" />
 <p className="font-mono text-gray-400">No assignments found. Start by creating one.</p>
 </div>
 ) : (
 assignments.map((asg) => (
 <div key={asg.id} className="bg-black border border-[#333] border border-[#333] p-6 rounded-sm hover: transition-all group">
 <div className="flex justify-between items-start mb-4">
 <h3 className="font-mono text-xl font-bold text-white line-clamp-1">{asg.title}</h3>
 <div className="flex gap-2">
 <button className="text-[#a855f7] hover:text-gray-400 transition-colors">
 <Eye size={18} />
 </button>
 </div>
 </div>
 
 <p className="text-gray-400 text-sm mb-6 line-clamp-3 h-15">{asg.description}</p>
 ...
 <span className="bg-[#111] text-[#a855f7] px-3 py-1 rounded-full text-[10px] font-mono font-bold tracking-widest uppercase">
 ACTIVE
 </span>
 <button 
 onClick={() => window.location.href = `/teacher/submissions?assignmentId=${asg.id}`}
 className="text-white font-mono text-xs font-bold hover:underline"
 >
 View Submissions →
 </button>
 </div>
